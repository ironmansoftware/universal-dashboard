...
namespace Microsoft.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsData.ConvertTo, "JsonEx", HelpUri = "https://go.microsoft.com/fwlink/?LinkID=217032", RemotingCapability = RemotingCapability.None)]
    [SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeSpelledCorrectly")]
    public class ConvertToJsonExCommand : PSCmdlet
    {
        private static readonly FieldInfo _immediateBaseObjectIsEmptyField = typeof(PSObject).GetField("immediateBaseObjectIsEmpty", BindingFlags.GetField | BindingFlags.NonPublic | BindingFlags.Instance);
        private static readonly MethodInfo _getPropertyCollectionMethod = typeof(PSObject).GetMethod("GetPropertyCollection", BindingFlags.InvokeMethod | BindingFlags.NonPublic | BindingFlags.Static, null, new[] { typeof(PSMemberViewTypes) }, null);
        private static readonly Type _psmemberInfoIntegrationCollection = typeof(PSObject).Assembly.GetType("System.Management.Automation.PSMemberInfoIntegratingCollection`1");
        ...
        protected override void BeginProcessing()
        {
            if (_depth > maxDepthAllowed)
            {
                ThrowTerminatingError(new ErrorRecord(
                                new InvalidOperationException("Reached maximum depth"),
            ...
            foreach (PropertyInfo info2 in t.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                if (!info2.IsDefined(typeof(T), true))
                {
                    ...
                    if ((getMethod != null) && (getMethod.GetParameters().Length <= 0))
                    {
                        ...
                        var jsonAttribute = (JsonPropertyAttribute)info2.GetCustomAttribute(typeof(JsonPropertyAttribute), true);
                        ...
                        {
                            result.Add(jsonAttribute.PropertyName, ProcessValue(value, depth + 1));
                        }
