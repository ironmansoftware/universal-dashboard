...
```
New-UDButton [[-Id] <String>] [[-Text] <Object>] [[-OnClick] <ScriptBlock>] [-Floating] [-Flat]
...
Creates a button that when clicked updates a label with the "Clicked!" text.

## PARAMETERS

### -Flat
Applies a flat style to the button.
...
### -Floating
Applies a float style to the button.
...
### -Icon
Embeds an icon in the button content.
...
### -IconAlignment
Aligns the icon on the left or right. 
...
### -OnClick
A script to invoke when the button is clicked.
