...
export default class App extends React.Component {
  ...
  componentWillMount() {
    this.loadJavascript(
      getApiPath() + "/api/internal/javascript/framework",
      function () {
        ...
        this.loadJavascript(
          getApiPath() + "/api/internal/javascript/plugin",
          function () {
            this.setState({
              loading: false,
            });
  ...
  render() {
    ...
    if (this.state.loading) {
      return (
        <div style={{ backgroundColor: "#FFFFFF" }} className="v-wrap">
          <article className="v-box">
            <Spinner
              name="folding-cube"
              style={{ width: "150px", height: "150px", color: "#0689B7" }}
            />
          </article>
        </div>
      );
